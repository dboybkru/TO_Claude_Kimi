...
export const journalsApi = {
  ...
  // Сводный журнал — Приложение №4 к ТЗ договора 10944505
  getSummary: (params?: { date_from?: string; date_to?: string; object_id?: string }) =>
    api.get<Record<string, unknown>[]>('/journals/summary', { params }).then((r) => r.data),
}
...
export const usersApi = {
  ...
  update: (id: string, data: UserUpdate) => api.put<User>(`/users/${id}`, data).then((r) => r.data),
...
export const voiceApi = {
  info: () => api.get<VoiceInfo>('/voice/info').then((r) => r.data),
